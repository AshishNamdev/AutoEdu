...
Description:
------------
This module automates the login process for the UDISE Student Module,
including handling CAPTCHA validation, credential verification,
academic year selection, and dismissal of school information pop-ups.
It is designed to support robust UI automation with retry logic
and logging for traceability.

Features:
---------
- Automated login with retry mechanism for invalid CAPTCHA or
...
- utils.utils.wait_and_click, wait_and_find_element: helper functions
    for UI interaction.
- ui.locators.udise.StudentImportLocator: locator definitions for UI elements.
- ui.udise.login: contains locator classes for academic year and
    school info elements.

Usage:
------
This module is intended to be used as part of a larger automation
...
class StudentLogin:
    ...
    This class provides methods to perform login operations with retry
    logic for invalid CAPTCHA or incorrect credentials, select the academic
    year post-login, and manage UI interruptions such as school information
    pop-ups.

    Methods:
    --------
    student_module_login(username, password, max_attempts=3):
        Attempts login with provided credentials, retrying on CAPTCHA
        or credential errors.

    select_academic_year():
        Selects the current academic year from the UI and closes any
        resulting pop-ups.

    close_school_info():
        Closes the school information dialog that may appear after
    ...
    Notes:
    ------
    - Relies on utility functions like `wait_and_click` and
        `wait_and_find_element`.
    - Uses predefined locators such as `StudentLogin.USERNAME`,
        `StudentLogin.PASSWORD`, etc.
    - Includes logging for traceability and debugging.
    """

    def student_login(self, username, password, max_attempts=3):
        """
        Attempts to log in to the UDISE student module with retries
        for invalid CAPTCHA or incorrect credentials.
    ...
    def select_academic_year(self):
        ...
        This function locates and clicks on the academic year element
        using predefined locators. After selection, it closes any school
        information pop-up or modal that may appear as a result
        of the selection.

        Dependencies:
            - wait_and_click: Utility function to wait for and
                click UI elements.
            - AcademicChoiceLocators.ACADEMIC_YEAR: Locator for the
                academic year element.
    ...
    def close_school_info(self):
        ...
        This function detects and dismisses the school information modal
        or overlay that may block further interaction with the page.
        It ensures smooth workflow by removing UI interruptions after
        academic year selection or login.

        Dependencies:
            - wait_and_click or equivalent method to interact with the
                close button.
            - Locator for the school info close button
                (e.g., SchoolInfoLocators.CLOSE_BUTTON).

        Returns:
            None

        Raises:
            Exception: If the pop-up cannot be closed due to missing
    ...
    def invalid_captcha(self):
        """
        Checks whether the CAPTCHA validation message indicates
        an invalid input.

        This function waits for the CAPTCHA message element to appear on the
        student login page, retrieves its inner HTML content, and returns
        True if the message contains the word "Invalid".
    ...
    def incorrect_creds(self):
        ...
        This function retrieves the inner HTML of the element associated with
        invalid login messages. It logs the message content and returns
        True if the message contains the keyword 'Incorrect',
